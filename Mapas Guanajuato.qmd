...
#municipios_guanajuato <- st_read("Mapa_Guanajuato/data/11mun.shp", options = "ENCODING=LATIN1", quiet = TRUE)

municipios_guanajuato <- st_read("data/11mun.shp", options = "ENCODING=LATIN1", quiet = TRUE)

# Crear mapa de municipios de Jalisco
mapa_municipios <- ggplot(municipios_guanajuato) +
  geom_sf(fill = "lightgreen", color = "white", size = 0.3) +
  labs(
    title = "Municipios de Guanajuato",
    subtitle = paste("Total de municipios:", nrow(municipios_guanajuato)),
...
Extraer el archivo `cpv_valor_11.csv` y guardarlo en la carpeta `data/`.
...
#datos_total <- read_csv("1_estadistica_descriptiva/data/cpv_valor_11.csv", na = c("ND"))

datos_total <- read_csv("cpv_valor_11.csv", na = c("ND"))
...
datos <- left_join(municipios_guanajuato, datos)
...
ggplot(datos) +
  ...
  labs(
    title = "Población de los Municipios de Guamajuato",
    caption = "Fuente: INEGI - Marco Geoestadístico"
  ) +
  theme_void() +
  theme(
