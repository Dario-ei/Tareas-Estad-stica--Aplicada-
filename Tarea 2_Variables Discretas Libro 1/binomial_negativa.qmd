...
```{r}
comparar_geom_nb <- function(r, p, n_sim = 10000) {
  set.seed(123) # reproducibilidad
  
  # 1. Simular suma de r geométricas
  simulaciones <- replicate(n_sim, sum(rgeom(r, prob = p) + 1)) 
  # +1 porque rgeom en R cuenta "fallas" antes del éxito
  
  # 2. Distribución teórica: Negativa binomial
  max_val <- max(simulaciones)
  valores <- r:r+max_val  # posibles valores
  probs_nb <- dnbinom(valores - r, size = r, prob = p) 
  
  # 3. Tabla de frecuencias observadas (simulaciones)
  frec_obs <- table(simulaciones) / n_sim
  
  # 4. Gráfico comparativo
  barplot(frec_obs, col = "skyblue", border = NA,
          main = paste("Comparación: suma de", r, "geométricas vs NegBin(r,p)"),
          xlab = "Valor", ylab = "Probabilidad")
  lines(valores, probs_nb, type = "b", col = "red", lwd = 2, pch = 19)
  legend("topright", legend = c("Simulación", "Teórica (NegBin)"),
         col = c("skyblue", "red"), lty = c(NA, 1), pch = c(15, 19), bty = "n")
}

# Ejemplo de uso:
comparar_geom_nb(r = 3, p = 0.4, n_sim = 5000)
