...
```{r}
# ------------------------------------------------------------
# Verificación de propiedades de la distribución F
# ------------------------------------------------------------
set.seed(123)
library(ggplot2)
...
df_a <- data.frame(val = X2)

ggplot(df_a, aes(x = val)) +
  geom_histogram(aes(y = ..density..), bins = 80, 
                 fill = "lightpink", color = "black", alpha = 0.6) +
  stat_function(fun = df, args = list(df1 = 1, df2 = n), 
                color = "red", size = 1.2) +
  xlim(0,10) +
  labs(title = paste("Verificación de que X^2 ~ F(1,", n, ")"),
       x = "x", y = "Densidad") +
  theme_minimal()
...
X_F <- rf(Nsim, df1 = a, df2 = b)
X_inv <- 1/X_F

df_b <- data.frame(val = X_inv)

ggplot(df_b, aes(x = val)) +
  geom_histogram(aes(y = ..density..), bins = 80, 
                 fill = "lightgreen", color = "black", alpha = 0.6) +
  stat_function(fun = df, args = list(df1 = b, df2 = a), 
                color = "red", size = 1.2) +
  xlim(0,5) +
  labs(title = paste("Verificación de que 1/X ~ F(", b, ",", a, ")"),
       x = "x", y = "Densidad") +
  theme_minimal()

# ------------------------------------------------------------
# c) Si X ~ Chi2(a), Y ~ Chi2(b) independientes,
#    entonces (X/a) / (Y/b) ~ F(a,b)
# ------------------------------------------------------------
a <- 5
b <- 10

X_chi <- rchisq(Nsim, df = a)
Y_chi <- rchisq(Nsim, df = b)

F_stat <- (X_chi / a) / (Y_chi / b)

df_c <- data.frame(val = F_stat)

ggplot(df_c, aes(x = val)) +
  geom_histogram(aes(y = ..density..), bins = 80, 
                 fill = "orange", color = "black", alpha = 0.6) +
  stat_function(fun = df, args = list(df1 = a, df2 = b), 
                color = "red", size = 1.2) +
  xlim(0,5) +
  labs(title = paste("Verificación de que (X/a)/(Y/b) ~ F(", a, ",", b, ")"),
       x = "x", y = "Densidad") +
  theme_minimal()
