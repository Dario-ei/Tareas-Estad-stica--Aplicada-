...
```{r}
# -----------------------------------------------------------
# Simulación con distribuciones Ji-Cuadrada
# -----------------------------------------------------------

# Parámetros
df_values <- c(5, 20, 50)           # grados de libertad
sample_sizes <- seq(100, 10000, by = 100)  # tamaños de muestra

# Función para graficar convergencia media y varianza
library(ggplot2)
...
for (df in df_values) {
  ...
  for (n in sample_sizes) {
    ...
    medias_muestrales <- c(medias_muestrales, mean(muestra))
    var_muestrales <- c(var_muestrales, var(muestra))
  }
  
  # 1. Convergencia de la media
  df_media <- data.frame(
    n = sample_sizes,
    media_muestral = medias_muestrales
  )
  
  p1 <- ggplot(df_media, aes(x = n, y = media_muestral)) +
    geom_line(color = "blue") +
    geom_hline(yintercept = media_teorica, color = "red", linetype = "dashed") +
    labs(title = paste("Convergencia de la media (df =", df, ")"),
         x = "Tamaño de muestra",
         y = "Media muestral") +
    theme_minimal()
  ...
  # 2. Convergencia de la varianza
  df_var <- data.frame(
    n = sample_sizes,
    var_muestral = var_muestrales
  )
  
  p2 <- ggplot(df_var, aes(x = n, y = var_muestral)) +
    geom_line(color = "darkgreen") +
    geom_hline(yintercept = var_teorica, color = "red", linetype = "dashed") +
    labs(title = paste("Convergencia de la varianza (df =", df, ")"),
         x = "Tamaño de muestra",
         y = "Varianza muestral") +
    theme_minimal()
  ...
  # 3. Histograma con densidad teórica (para n = 10000)
  muestra_final <- rchisq(10000, df = df)
  df_hist <- data.frame(x = muestra_final)
  
  p3 <- ggplot(df_hist, aes(x)) +
    geom_histogram(aes(y = ..density..), bins = 50, fill = "skyblue", color = "black") +
    stat_function(fun = dchisq, args = list(df = df), color = "red", size = 1.2) +
    labs(title = paste("Histograma y densidad teórica (df =", df, ", n=10000)"),
         x = "Valor", y = "Densidad") +
    theme_minimal()
...
```{r}
# -----------------------------------------------------------
# Verificación del Teorema Central del Límite con Ji-Cuadrada
# -----------------------------------------------------------
...
mu <- df                # media teórica
sigma <- sqrt(2 * df)   # desviación estándar teórica
nsim <- 10000           # número de simulaciones
sample_sizes <- c(5, 30, 100)   # tamaños de muestra a comparar
...
for (n in sample_sizes) {
  # Repetimos nsim veces: sacar muestra, calcular media y estandarizar
  Z <- replicate(nsim, {
    muestra <- rchisq(n, df = df)
    media_m <- mean(muestra)
    (media_m - mu) / (sigma / sqrt(n))
  })
  
  # Guardar resultados
  resultados <- rbind(resultados,
                      data.frame(Z = Z, n = as.factor(n)))
}

# Graficar histogramas comparando con N(0,1)
ggplot(resultados, aes(x = Z, fill = n)) +
  geom_histogram(aes(y = ..density..), bins = 40, alpha = 0.6, color = "black") +
  stat_function(fun = dnorm, args = list(mean = 0, sd = 1),
                color = "red", size = 1.2) +
  facet_wrap(~n, scales = "free") +
  labs(title = paste("Verificación del TCL con Ji-Cuadrada (df =", df, ")"),
       x = "Valores estandarizados Z", y = "Densidad") +
  theme_minimal()
...
```{r}
# -----------------------------------------------------------
# Verificación de la propiedad de aditividad del Ji-Cuadrada
# -----------------------------------------------------------

set.seed(123)
nsim <- 100000     # número de simulaciones
df_values <- c(2, 5, 10)  # grados de libertad a comparar

library(ggplot2)

for (df in df_values) {
  # Generar normales estándar N(0,1)
  Z <- matrix(rnorm(nsim * df, mean = 0, sd = 1), ncol = df)
  
  # Sumar cuadrados
  chi_sim <- rowSums(Z^2)
  
  # Pasar a data frame
  df_data <- data.frame(x = chi_sim)
  
  # Graficar histograma vs densidad teórica
  p <- ggplot(df_data, aes(x)) +
    geom_histogram(aes(y = ..density..), bins = 60, 
                   fill = "darkgreen", color = "black", alpha = 0.6) +
    stat_function(fun = dchisq, args = list(df = df),
                  color = "red", size = 1.2) +
    labs(title = paste("Suma de cuadrados de normales estándar vs Ji-Cuadrada (df =", df, ")"),
         x = "Valor", y = "Densidad") +
    theme_minimal()
...
```{r}
# Verificación numérica de la aproximación normal de la Ji-Cuadrada
set.seed(2025)
...
# Funciones auxiliares para skewness y kurtosis (exceso)
skewness <- function(x) {
  m3 <- mean((x - mean(x))^3)
  s3 <- sd(x)^3
  m3 / s3
...
# Parámetros
df_values <- c(10, 30, 50, 100)    # grados de libertad a evaluar
nsim <- 100000                     # tamaño de muestra para la simulación por df

# Resultados numéricos
res_tabla <- data.frame(df = integer(),
                        n = integer(),
                        media_emp = double(),
                        var_emp = double(),
                        skew_emp = double(),
                        kurt_exceso_emp = double(),
                        KS_stat = double(),
                        KS_pvalue = double(),
...
for (k in df_values) {
  ...
  # Estandarizar para comparar con N(0,1)
  z <- (x - mu) / sd_teor
  
  # KS test contra N(0,1) (uso pnorm)
  ks <- ks.test(z, "pnorm", mean = 0, sd = 1)
  
  # Guardar en tabla
  res_tabla <- rbind(res_tabla,
                     data.frame(df = k,
                                n = nsim,
                                media_emp = media_emp,
                                var_emp = var_emp,
                                skew_emp = skew_emp,
                                kurt_exceso_emp = kurt_emp,
                                KS_stat = ks$statistic,
                                KS_pvalue = ks$p.value))
  
  # --- Gráficas ---
  # 1) Histograma con densidad normal N(k, 2k)
  df_plot <- data.frame(x = x)
  p1 <- ggplot(df_plot, aes(x = x)) +
    geom_histogram(aes(y = ..density..), bins = 80, fill = "lightblue", color = "black", alpha = 0.6) +
    stat_function(fun = dnorm, args = list(mean = mu, sd = sd_teor), color = "red", size = 1) +
    labs(title = paste0("Chi-square(df=", k, ") vs Normal N(", k, ",", round(var_teor,2),")"),
         x = "x", y = "Densidad") +
    theme_minimal()
  
  # 2) Histograma de Z con N(0,1)
  df_z <- data.frame(z = z)
  p2 <- ggplot(df_z, aes(x = z)) +
    geom_histogram(aes(y = ..density..), bins = 80, fill = "lightgreen", color = "black", alpha = 0.6) +
    stat_function(fun = dnorm, args = list(mean = 0, sd = 1), color = "red", size = 1) +
    labs(title = paste0("Estandarizado Z = (X - ", k, ") / sqrt(2*", k, ")"),
         x = "Z", y = "Densidad") +
    theme_minimal()
  
  # 3) QQ-plot contra N(0,1)
  qd <- qqnorm(z, plot.it = FALSE)
  qqdf <- data.frame(theo = qd$x, samp = qd$y)
  p3 <- ggplot(qqdf, aes(x = theo, y = samp)) +
    geom_point(alpha = 0.4, size = 0.6) +
    geom_abline(intercept = 0, slope = 1, color = "red", linetype = "dashed") +
    labs(title = paste0("QQ-plot of Z vs N(0,1) (df=", k, ")"),
         x = "Theoretical quantiles", y = "Sample quantiles") +
    theme_minimal()
  
  # Mostrar las 3 gráficas en un panel (puedes comentarlo si prefieres verlas separadas)
  grid.arrange(p1, p2, p3, ncol = 1)
  
  # Imprimir resumen corto en consola
  cat("\n--- df =", k, "---\n")
  cat("Media empírica:", round(media_emp,4), " | Media teórica:", mu, "\n")
  cat("Var empírica:", round(var_emp,4), " | Var teórica:", round(var_teor,4), "\n")
  cat("Skewness empírica:", round(skew_emp,4), " | Curtosis exceso empírica:", round(kurt_emp,4), "\n")
  cat("KS stat (Z vs N(0,1)):", round(ks$statistic,5), " | p-value:", signif(ks$p.value,4), "\n")
}

# Mostrar tabla resumen con formato legible
print(res_tabla)
...
```{r}
# -----------------------------------------------------------
# Verificación de aditividad en Ji-Cuadrada
# -----------------------------------------------------------

set.seed(123)
nsim <- 100000   # número de simulaciones

# Grados de libertad de cada chi-cuadrada
df1 <- 5
df2 <- 10

# Simulaciones independientes
X1 <- rchisq(nsim, df = df1)
X2 <- rchisq(nsim, df = df2)

# Suma
X_sum <- X1 + X2

# Teórica: chi-cuadrada con df = df1 + df2
df_total <- df1 + df2
mu_teor <- df_total
var_teor <- 2 * df_total

# Estadísticos empíricos
media_emp <- mean(X_sum)
var_emp <- var(X_sum)

cat("Resultados empíricos vs teóricos\n")
cat("Media empírica: ", round(media_emp,4), " | Media teórica: ", mu_teor, "\n")
cat("Varianza empírica: ", round(var_emp,4), " | Varianza teórica: ", var_teor, "\n")

# Comparar gráficamente
library(ggplot2)

df_data <- data.frame(x = X_sum)

ggplot(df_data, aes(x)) +
  geom_histogram(aes(y = ..density..), bins = 80,
                 fill = "darkgreen", color = "black", alpha = 0.6) +
  stat_function(fun = dchisq, args = list(df = df_total),
                color = "red", size = 1.2) +
  labs(title = paste("Aditividad: X1 ~ Chi2(", df1, "), X2 ~ Chi2(", df2, ")\n",
                     "Suma ~ Chi2(", df_total, ")", sep = ""),
       x = "Valor", y = "Densidad") +
  theme_minimal()
